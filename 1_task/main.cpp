...
complexd* generate_condition(int n){
    ...
    double module =0;
#pragma omp parallel shared(V) firstprivate(time_init) reduction(+: module)
    {
...
int main(int argc , char** argv) {
    ...
    cout << '~' << n << '\t' << k << '\t';
    printf(" LIFETIME:%lf s ",(float)((stop.tv_sec - start.tv_sec)*1000000 + stop.tv_usec - start.tv_usec)/1000000);
    delete [] V;
    delete [] W;
